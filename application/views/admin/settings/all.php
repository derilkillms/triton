                                        ...
                                        <div>
                                            ...
                                            <ul class="tw-space-y-2">
                                                <?php foreach ($section['children'] as $child) { ?>
                                                    <li class="settings-group-<?= e($child['id']); ?>">
                                                        <a href="<?= admin_url('settings?group=' . $child['id']); ?>" class="tw-group tw-flex tw-items-center tw-text-sm hover:tw-text-neutral-800 focus:tw-text-neutral-800 tw-font-medium tw-gap-2.5 <?= ($group['id'] === $child['id']) ? ' tw-text-neutral-800' : 'tw-text-neutral-600' ?>">
                                                            <i class="<?= $child['icon'] ?? 'fa-regular fa-circle-question'; ?> fa-fw fa-lg tw-mr-0.5 group-hover:tw-text-neutral-800 <?= $group['id'] === $child['id'] ? 'tw-text-neutral-800' : 'tw-text-neutral-500'; ?>"></i>
                                                            <span>
                                                                <?= $child['name']; ?>
                                                            </span>
                                                            <?php if (isset($child['badge'], $child['badge']['value']) && !empty($child['badge'])) { ?>
                                                                <span class="badge tw-ml-auto
        <?= isset($child['badge']['type']) && $child['badge']['type'] != '' ? "bg-{$child['badge']['type']}" : 'bg-info' ?>" <?= (isset($child['badge']['type']) && $child['badge']['type'] == '') || isset($child['badge']['color']) ? "style='background-color: {$child['badge']['color']}'" : '' ?>>
                                                                    <?= $child['badge']['value'] ?>
                                                                </span>
                                                            <?php } ?>
                                                        </a>
                                                ...
                                                <?php if ($sectionId === 'general') { ?>
                                                    <li class="settings-group-system-update">
                                                        <a href="<?= admin_url('settings?group=update'); ?>" class="tw-group tw-flex tw-items-center tw-text-sm hover:tw-text-neutral-800 focus:tw-text-neutral-800 tw-font-medium tw-gap-2.5 <?= ($group['id'] === 'update') ? ' tw-text-neutral-800' : 'tw-text-neutral-600' ?>">
                                                            <i class="fa-solid fa-hammer fa-fw fa-lg tw-mr-0.5 group-hover:tw-text-neutral-800 <?= $group['id'] === 'update' ? 'tw-text-neutral-800' : 'tw-text-neutral-500'; ?>"></i>
                                                    ...
                                                    <?php if (is_admin()) { ?>
                                                        <li class="settings-group-system-info">
                                                            <a href="<?= admin_url('settings?group=info'); ?>" class="tw-group tw-flex tw-items-center tw-text-sm hover:tw-text-neutral-800 focus:tw-text-neutral-800 tw-font-medium tw-gap-2.5 <?= ($group['id'] === 'info') ? ' tw-text-neutral-800' : 'tw-text-neutral-600' ?>">
                                                                <i class="fa-solid fa-question fa-fw fa-lg tw-mr-0.5 group-hover:tw-text-neutral-800 <?= $group['id'] === 'update' ? 'tw-text-neutral-800' : 'tw-text-neutral-500'; ?>"></i>
