<script>
    ...
    if (pur_request != '') {
      $.post(admin_url + 'purchase/coppy_pur_request_for_po/' + pur_request + '/' + vendor).done(function(response) {
        ...
        if (response) {
          ...
          // Hapus hanya data yang berasal dari pur_request
          $('.invoice-item table.invoice-items-table.items tbody tr[data-source="pur_request"]').remove();

          // Tambahkan item baru dengan atribut data-source="pur_request"
          let newItems = $(response.list_item);
          newItems.attr("data-source", "pur_request");
          $('.invoice-item table.invoice-items-table.items tbody').append(newItems);
  ...
  function coppy_project() {
    "use strict";
    var project = $('select[name="project"]').val();
    var vendor = $('select[name="vendor"]').val();
    if (project != '') {
      $.post(admin_url + 'purchase/coppy_project_for_po/' + project + '/' + vendor).done(function(response) {
        response = JSON.parse(response);
        if (response) {
          $('select[name="estimate"]').html(response.estimate_html);
          $('select[name="estimate"]').selectpicker('refresh');
          ...
          // Hapus hanya data yang berasal dari project
          $('.invoice-item table.invoice-items-table.items tbody tr[data-source="project"]').remove();

          // Tambahkan item baru dengan atribut data-source="project"
          let newItems = $(response.list_item);
          newItems.attr("data-source", "project");
          $('.invoice-item table.invoice-items-table.items tbody').append(newItems);
    ...
    if (sale_invoice != '') {
      $.post(admin_url + 'purchase/coppy_sale_invoice_po/' + sale_invoice).done(function(response) {
        ...
        if (response) {
          ...
          // Hapus hanya data yang berasal dari project
          $('.invoice-item table.invoice-items-table.items tbody tr[data-source="sale_invoice"]').remove();

          // Tambahkan item baru dengan atribut data-source="project"
          let newItems = $(response.list_item);
          newItems.attr("data-source", "sale_invoice");
          $('.invoice-item table.invoice-items-table.items tbody').append(newItems);
