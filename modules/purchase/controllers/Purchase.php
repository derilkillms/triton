...
class purchase extends AdminController
{
    ...
    public function view_pur_request($id)
    {
        ...
        $setting = $this->purchase_model->get_setting(); 
        ...
        $data['comments'] = $this->purchase_model->get_request_comments($id);
        $this->load->view('purchase_request/view_pur_request', $data);
    }

    public function list_request_comment($rel_id) {
        if (!has_permission('purchase_request', '', 'view') && !has_permission('purchase_request', '', 'view_own')) {
            access_denied('purchase');
        }
        ...
        $this->load->model('departments_model');
        $this->load->model('currencies_model');
        $data['comments'] = $this->purchase_model->get_request_comments($rel_id);
        $this->load->view('purchase_request/comment_list', $data);
    }

    public function add_request_comment() {
        $data = [
            'content'   => $this->input->post('content'),
            'rel_id'    => $this->input->post('rel_id'),
            'rel_type'  => 'pur_request',
            'staffid'   => get_staff_user_id(),
            'dateadded' => date('Y-m-d H:i:s')
        ];
        $this->purchase_model->insert_request_comment($data);
        echo json_encode(['success' => true]);
    }

    public function edit_request_comment() {
        $id = $this->input->post('id');
        $data = ['content' => $this->input->post('content')];
        $this->purchase_model->update_request_comment($id, $data);
        echo json_encode(['success' => true]);
    }

    public function delete_request_comment() {
        $id = $this->input->post('id');
        $this->purchase_model->delete_request_comment($id);
        echo json_encode(['success' => true]);
    }
    ...
    {
        // $this->load->model('Purchase_model');
        $data['get_setting'] = $this->purchase_model->get_setting(); 
        $this->app->get_table_data(module_views_path('purchase', 'purchase_request/table_pur_request'),$data);
    }
    ...
    public function get_pur_order_data_ajax($id, $to_return = false)
    {
        ...
        $setting = $this->purchase_model->get_setting('pur_order'); 
        ...
        if (is_numeric($id)) {
                ...
                if ($key == 0) {
                    $html .= '<div id="item_approve">
                        ...
                        render_select('approver[' . $key . ']', $approver, array('id', 'name'), 'approver', $value->approver, array('data-id' => '0', 'required' => 'true'), [], '', 'approver_class') . '
                                    ...
                                    <div class="col-md-4 ' . $staff_hide . '" id="is_staff_0">
                                    ' . render_select('staff[' . $key . ']', $staffs, array('staffid', 'full_name'), 'staff', $value->staff) . '
                                    </div>
                                    <div class="col-md-4" id="is_departmen_0">
                                    ' . render_select('department[0]', $departements, array('departmentid', 'name'), 'Department', $value->department) . '
                                    </div>
                                    <div class="col-md-3">
            ...
            $html .= '<div id="item_approve">
                ...
                render_select('approver[0]', $approver, array('id', 'name'), 'approver', $approver_default, array('data-id' => '0', 'required' => 'true'), [], '', 'approver_class') . '
                        ...
                        <div class="col-md-4" id="is_departmen_0">
                        ' . render_select('department[0]', $departements, array('departmentid', 'name'), 'Department') . '
                        </div>
                        <div class="col-md-3">
