...
// print_r(($get_setting)); die();

$setting_staff = json_decode($get_setting,true);
// print_r($setting_staff); die();
...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if (has_access(get_staff_user_id(), $aRow['department'], $setting_staff)) {
            if(has_permission('purchase_request_change_approve_status', '', 'edit')){
                ...
                if($aRow['status'] == 1){
                    ...
                    $approve_status .= '<li>
                              <a href="#" onclick="change_pr_approve_status( 0 ,' . $aRow['id'] . '); return false;">
                                 ' ._l('pur_rejected') . '
                              </a>
                           </li>';
                ...
                // $_data .= '<button>Edit</button>'; 
            }

            $approve_status .= '</span>';

            if($aRow['status'] == 0){
                $approve_status = '<span class="label label-danger" id="status_span_'.$aRow['id'].'"> Void';
            }
