...
class Purchase_model extends App_Model
{
    ...
    public function add_vendor($data, $client_id = null, $client_or_lead_convert_request = false) {
        ...
        // Inisialisasi variabel untuk file NPWP dan NIB
        $npwp_file = '';
        $nib_file  = '';
        ...
        if (!empty($_FILES['npwp']['name'])) {
            if ($this->upload->do_upload('npwp')) {
                ...
                $npwp_file = $upload_data['file_name'];
            } else {
        ...
        if (!empty($_FILES['nib_file']['name'])) {
            if ($this->upload->do_upload('nib_file')) {
                $upload_data = $this->upload->data();
                $nib_file = $upload_data['file_name'];
            } else {
                echo $this->upload->display_errors();
                return false;
            }
        ...
        // Tambahkan data NPWP dan NIB ke dalam array data vendor
        $data['npwp']     = $npwp_file;
        $data['nib_file'] = $nib_file;

        // Proses data vendor seperti sebelumnya
    ...
    public function add_approval_setting($data)
    {
        ...
        if (isset($data['approver'])) {
            ...
            foreach ($data['approver'] as $key => $value) {
                ...
                $node['department'] = $data['department'][$key];
    ...
    public function get_request_comments($rel_id) {
        return $this->db->get_where('tblpur_comments', ['rel_type' => 'pur_request', 'rel_id' => $rel_id])->result();
    }

    public function insert_request_comment($data) {
        $this->db->insert('tblpur_comments', $data);
    }

    public function update_request_comment($id, $data) {
        $this->db->where('id', $id)->update('tblpur_comments', $data);
    }

    public function delete_request_comment($id) {
        $this->db->where('id', $id)->delete('tblpur_comments');
    }
}
